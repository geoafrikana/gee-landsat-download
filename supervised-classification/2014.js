...
var training_2014 = bare_soil.merge(dense_vegetation).merge(light_vegetation)
                      .merge(water).merge(built_area)

Export.table.toAsset({
  collection: training_2014,
  description: 'training_2014_export',
  assetId: 'projects/ornate-lead-196416/assets/training_2014'
});
...
// Add Layers to Map
// ================================
Map.addLayer(img2014, vis, '2014 Composite');
Map.addLayer(img2014.select('NDVI'), {
  min: -0.2,
  max: 0.8,
  palette: [
...
var training = ee.FeatureCollection('projects/ornate-lead-196416/assets/training_2014');


var label = 'Class';
var bands = ['Blue', 'Green', 'Red', 'NIR',
              'SWIR1', 'NDVI', 'MNDWI', 'NDBI']; 
var input = img2014.select(bands);
print(img2014)
